...
static SPI_Config_t MPU9250_SPI = {
  .Channel = MPU9250_SPI_CHANNEL, .CPHA = SPI_CPHA_2EDGE, .CPOL = SPI_CPOL_HIGH, .Mode = SPI_MODE_MASTER, .BaudRatePrescaler = SPI_BAUDRATEPRESCALER_64, .SPE = SPI_SPE_EN, .BFIRST = SPI_MSBFIRST, .NSS_MAN = SPI_NSS_SOFTWARE, .NSSI_MODE = SPI_NSSI_NOT_SELECT, /* Fixed Master Mode Fault */
  .DFF = SPI_DFF_8BIT,
  .CRC_MODE = SPI_CRCDIS
};
...
static void MPU9250_vSelect(void)
{
  GPIO_enumWritePinVal(MPU9250_CS_PORT, MPU9250_CS_PIN, GPIO_PIN_LOW);
}
/** @brief Release chip-select to end an SPI transaction. */
static void MPU9250_vDeselect(void)
{
  GPIO_enumWritePinVal(MPU9250_CS_PORT, MPU9250_CS_PIN, GPIO_PIN_HIGH);
}
...
static void MPU9250_vInitMag(void)
{
  ...
  MPU9250_vWriteReg(0x37, 0x00);
  TIM_vDelayMs(MPU9250_DELAY_TIMER, 10);
  MPU9250_vWriteReg(0x6A, 0x30);
  MPU9250_vWriteReg(0x24, 0x0D);
...
ErrorState_t MPU9250_enumInit(void)
{
  ...
  MPU9250_vWriteReg(0x6B, 0x80);
  TIM_vDelayMs(MPU9250_DELAY_TIMER, 150);
  MPU9250_vWriteReg(0x6B, 0x01);
  TIM_vDelayMs(MPU9250_DELAY_TIMER, 50);
  ...
  for (int i = 0; i < 400; i++)
  {
    ...
    uint16_t v;
    MPU9250_vSelect();
    SPI_enumTrancieve(&MPU9250_SPI, 0xBB, &v);
    for (int j = 0; j < 14; j++)
    {
      SPI_enumTrancieve(&MPU9250_SPI, 0, &v);
      b[j] = (uint8_t)v;
    }
    MPU9250_vDeselect();
...
ErrorState_t MPU9250_enumReadData(MPU9250_Data_t *D)
{
  ...
  uint16_t val;
  MPU9250_vSelect();
  SPI_enumTrancieve(&MPU9250_SPI, 0xBB, &val);
  for (int i = 0; i < 21; i++)
  {
    SPI_enumTrancieve(&MPU9250_SPI, 0, &val);
