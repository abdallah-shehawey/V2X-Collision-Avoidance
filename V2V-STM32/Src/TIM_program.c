 ...
 */
static void (*TIM_CC_Callbacks[TIM_TIMER_COUNT])(TIM_Num_t, uint8_t, uint32_t) = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };
...
{
  ErrorState_t Local_ErrorState = OK;
  if (Copy_eTimer >= TIM_TIMER_COUNT || pu32Value == NULL)
    return NOK;
...
ErrorState_t TIM_vSetICPolarity(TIM_Num_t Copy_eTimer, TIM_Channel_t Copy_eChannel, TIM_Polarity_t Copy_ePolarity)
{
  ...
  {
  case TIM_CHANNEL1:
    if (Copy_ePolarity == TIM_POLARITY_LOW)
      SET_BIT(TIMx->CCER, TIM_CCER_CC1P);
    else
      CLR_BIT(TIMx->CCER, TIM_CCER_CC1P);
    break;
...
{
  ErrorState_t Local_ErrorState = OK;
  if (Copy_eTimer >= TIM_TIMER_COUNT || pvCallback == NULL)
    return NOK;
...
{
  if (Copy_eTimer >= TIM_TIMER_COUNT || Copy_eChannel > TIM_CHANNEL4)
    return NOK;
