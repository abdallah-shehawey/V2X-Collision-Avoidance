...
// ==================== V2N Panel (Traffic Light + Ambulance) ====================
const TL_COLORS = { RED: "#ff4d4f", GREEN: "#2ecc71", YELLOW: "#f5a623" };

function renderV2N(v2n) {
  if (!v2n) return;
  const tl   = v2n.trafficLight || {};
  const amb  = v2n.ambulance    || {};
  const state = (tl.state || "RED").toUpperCase();
  const color = TL_COLORS[state] || "#888";

  // Traffic light bulbs
  ["RED","GREEN","YELLOW"].forEach(s => {
    const el = $("tl-" + s.toLowerCase());
    if (el) el.style.opacity = (s === state) ? "1" : "0.15";
  });

  // State label + countdown
  const lbl = $("tl-label");
  if (lbl) { lbl.textContent = state; lbl.style.color = color; }
  const ct = $("tl-countdown");
  if (ct) {
    const t = tl.remainingTime;
    ct.textContent = (t != null && t > 0) ? `${t}s` : "--";
    ct.style.color = color;
  }

  // Emergency banner
  const banner = $("v2n-emergency");
  if (banner) {
    if (tl.isEmergency) {
      banner.style.display = "flex";
      const msg = $("v2n-emergency-msg");
      if (msg) msg.textContent = tl.warning || "EMERGENCY";
    } else {
  ...
  const ambCount = $("amb-count");
  const isAmb    = amb.vehicleType === "ambulance";

  if (ambType)  { ambType.textContent  = isAmb ? "🚑 AMBULANCE" : "🚗 Normal"; ambType.style.color = isAmb ? "var(--crit)" : "var(--safe)"; }
  if (ambId)    ambId.textContent    = amb.vehicleId   || "--";
  if (ambDist)  ambDist.textContent  = amb.distanceM != null ? `${amb.distanceM.toFixed(0)} m` : "--";
  if (ambCount) ambCount.textContent = `${amb.nearbyCount || 0} nearby`;
}

// ==================== V2P Panel (Pedestrians + Vehicle Sensing) ====================
const ALERT_COLORS = { none: "var(--safe)", caution: "var(--warn)", danger: "var(--crit)" };
const ALERT_LABELS = { none: "CLEAR", caution: "CAUTION", danger: "DANGER" };

function renderV2P(v2p) {
  if (!v2p) return;
  const ped  = v2p.pedestrians    || {};
  const vs   = v2p.vehicleSensing || {};
  const alert = (ped.alertLevel || "none").toLowerCase();
  const color = ALERT_COLORS[alert] || ALERT_COLORS.none;

  // Pedestrian counts
  const pedCount  = $("v2p-ped-count");
  const pedMoving = $("v2p-ped-moving");
  const pedSafe   = $("v2p-ped-safe");
  const pedAlert  = $("v2p-alert");

  if (pedCount)  pedCount.textContent  = ped.count       || 0;
  if (pedMoving) pedMoving.textContent = `${ped.movingCount || 0} moving`;
  if (pedSafe) {
    const safe = ped.crossingSafe !== false;
    pedSafe.textContent = safe ? "✔ Crossing Safe" : "✘ Crossing Unsafe";
    pedSafe.style.color = safe ? "var(--safe)" : "var(--crit)";
  }
  if (pedAlert) {
    pedAlert.textContent = ALERT_LABELS[alert] || "CLEAR";
    pedAlert.style.color = color;
    pedAlert.className = `v2p-alert-badge ${alert}`;
  }

  // Traffic light shown by V2P (what the camera side sees)
  const v2pLight = $("v2p-light");
  if (v2pLight) {
    const ls = (v2p.trafficLightState || "RED").toUpperCase();
    v2pLight.textContent = ls;
    v2pLight.style.color = TL_COLORS[ls] || "#888";
  }

  // Vehicle sensing
  const vsSpeed  = $("vs-speed");
  const vsCars   = $("vs-cars");
  const vsBikes  = $("vs-bikes");
  const vsFront  = $("vs-front");
  const vsBrake  = $("vs-brake");

  if (vsSpeed) vsSpeed.textContent = `${vs.speedKmh   || 0} km/h`;
  if (vsCars)  vsCars.textContent  = `${vs.carCount   || 0} cars`;
  if (vsBikes) vsBikes.textContent = `${vs.bikeCount  || 0} bikes`;
  if (vsFront) {
    const cm = vs.frontCm;
    vsFront.textContent = (cm && cm < 999) ? `${cm.toFixed(0)} cm` : "clear";
    vsFront.style.color = (cm < 50) ? "var(--crit)" : (cm < 120) ? "var(--warn)" : "var(--safe)";
  }
  if (vsBrake) {
    vsBrake.textContent = vs.brakePressed ? "⚠ BRAKING" : "Normal";
    vsBrake.style.color = vs.brakePressed ? "var(--crit)" : "var(--safe)";
  }
}
