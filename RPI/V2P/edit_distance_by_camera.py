# -*- coding: utf-8 -*-
"""edit_distance_by_camera.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16C0CdW2d7nyhMZfaXaY2Kitdcwgctpl4
"""
...
print("="*60)
print("🚗 REAL-TIME V2X CAMERA NODE (LOCAL MODE) - DISTANCE FIXED + PAUSE")
print("="*60)
...
MIN_CAR_WIDTH = 150
SPEED_ASSUMED = 5.0         # Assumed speed for ambulance passing (m/s)
...
# ============================================
# LICENSE PLATE DETECTION & OCR (MODIFIED)
# ============================================

def process_frame(frame, detected_plates):
    """
    Detect cars, read plates using detail=1 to get actual plate width,
    and return pause duration if an ambulance is detected.
    """
    results = car_detector(frame, verbose=False, conf=0.5)
    pause_duration = 0.0  # Default: no pause
                ...
                if plate_region.size > 0:
                    ...
                    for (bbox, text, conf) in ocr_results:
                        ...
                        if len(cleaned) >= 4:
                            ...
                            # ============================================================
                            # 🚑 إذا كانت سيارة إسعاف، نحسب زمن الإغلاق
                            # ============================================================
                            if is_ambulance and distance != float('inf'):
                                # تقدير الزمن = المسافة / السرعة الافتراضية (مع حد أدنى 2 ثانية وأقصى 20)
                                pause_duration = max(2.0, min(20.0, distance / SPEED_ASSUMED))
                                # نطبع مرة واحدة فقط، لكن نكمل معالجة باقي السيارات في نفس الإطار
                                print(f"⏳ Ambulance detected at {distance_text} → Will pause for {pause_duration:.1f}s")

                            # نكتفي بلوحة واحدة لكل سيارة (أول لوحة تظهر)
                            break

    return frame, pause_duration

# ============================================
# LIVE CAMERA PIPELINE (LOCAL PC) - WITH PAUSE LOGIC
# ============================================
def start_local_camera():
    ...
    # جلب سرعة الإطارات (FPS) من الكاميرا
    fps = cap.get(cv2.CAP_PROP_FPS)
    if fps <= 0:
        fps = 30  # قيمة افتراضية إذا لم تدعم الكاميرا الإبلاغ عن الـ FPS

    print("\n📸 Live Camera Feed Started!")
    print("👉 Press the 'q' key on your keyboard to stop the camera and exit safely.")
    print(f"⚙️  FPS: {fps:.1f} | Ambulance Pause: ENABLED (Speed assumed: {SPEED_ASSUMED} m/s)")
    print("="*60)
    ...
    # ============================================================
    # 🕒 متغيرات التحكم في إغلاق الكاميرا المؤقت
    # ============================================================
    pause_mode = False
    ...
    try:
        while True:
            ...
            # ============================================================
            # 🚫 إذا كنا في وضع الإغلاق، نعرض الإطار فقط دون معالجة
            # ============================================================
            if pause_mode:
                pause_frames_remaining -= 1
                if pause_frames_remaining <= 0:
                    pause_mode = False
                    print("🔓 Camera resumed (pause ended).")
                # نعرض الإطار ونكمل الحلقة بدون معالجة
            ...
            # ============================================================
            # 🔍 المعالجة العادية (كل 5 إطارات لتخفيف الحمل)
            # ============================================================
            if frame_count % 5 == 0:
                processed_frame, pause_duration = process_frame(frame, detected_plates)
                frame = processed_frame  # تحديث الإطار بالرسومات

                # إذا طلبت الدالة إيقافاً مؤقتاً، نفعله
                if pause_duration > 0:
                    pause_frames = int(pause_duration * fps)
                    pause_mode = True
                    pause_frames_remaining = pause_frames
                    print(f"⏸️ Camera PAUSED for {pause_duration:.1f} seconds ({pause_frames} frames)")
