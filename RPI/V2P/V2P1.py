...
config = picam2.create_preview_configuration(
    ...
    # actually delivers pixels in BGR byte order, and "BGR888" actually
    # delivers true RGB order. Using "RGB888" here was causing the
    # red/blue channels to be swapped once by the camera itself, then
    # swapped AGAIN by the cv2.cvtColor(..., COLOR_RGB2BGR) call below —
    # two swaps cancel out for grayscale but not for real colors, which
    # is why colors looked wrong (reds/blues flipped). Using "BGR888"
    # gives data that is already in the order OpenCV expects, so no
...
try:
    while True:
        ...
        if frame_count % skip_frames == 0:
            # The ONNX model (like most YOLO-family models) was trained on
            # RGB-ordered images, so convert the display frame (now BGR,
            # correct for cv2.imshow) to RGB just for the inference blob.
