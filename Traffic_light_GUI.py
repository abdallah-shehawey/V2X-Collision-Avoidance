...
class TrafficLightGUI:
    ...
    def __init__(self, root: tk.Tk):
        ...
        # V2X Preemption States
        self.in_preemption = False
        self.preempt_yellow_active = False
        self.preempt_saved_state = "RED"
        self.preempt_saved_remaining_time = 0
        self.preempt_saved_sim_index = 0
        self.preempt_saved_active = False
        self.preempt_extended = False
        self.preempt_off_delay_id = None  # Debounce timer id for preemption clearing
        self.preempt_extended = False
    ...
    def _on_mqtt_message(self, client, userdata, msg):
        """
        Processes inbound processed traffic messages from the central Gateway.
        """
        try:
            ...
            if msg.topic == PROCESSED_TOPIC:
                ...
                self.root.after(0, lambda: self.handle_preemption_signal(is_emergency))
        except Exception as e:
            print(f"⚠️ Error parsing message in Traffic Light GUI: {e}")

    def handle_preemption_signal(self, is_emergency: bool):
        """
        Handles state machine overrides based on active emergency vehicle preemption status.
        """
        if is_emergency:
            if self.preempt_off_delay_id is not None:
                print("🚨 Preemption exit cancelled due to ambulance re-detection.")
                self.root.after_cancel(self.preempt_off_delay_id)
                self.preempt_off_delay_id = None
                
            if not self.in_preemption:
                self.in_preemption = True
                self.preempt_saved_state = self.state
                self.preempt_saved_remaining_time = self.remaining_time
                self.preempt_saved_sim_index = self.sim_index
                self.preempt_saved_active = self.simulation_active
                self.preempt_extended = False
                
                print(f"🚨 Ambulance Detected! Saving state: {self.state} ({self.remaining_time}s left)")
                
                # Pause/cancel normal simulation scheduled ticks
                if self.sim_after_id:
                    self.root.after_cancel(self.sim_after_id)
                    self.sim_after_id = None
                
                if self.state == "RED":
                    print("🚦 Current state is RED -> Transitioning to YELLOW (3 seconds)...")
                    self.preempt_yellow_active = True
                    self._apply_update("YELLOW", 3)
                    self._run_preempt_tick(3)
                elif self.state in ("YELLOW", "AMBER"):
                    print(f"🚦 Current state is YELLOW ({self.remaining_time}s left) -> Letting yellow finish...")
                    self.preempt_yellow_active = True
                    self._apply_update("YELLOW", self.remaining_time)
                    self._run_preempt_tick(self.remaining_time)
                elif self.state == "GREEN":
                    print(f"🚦 Current state is GREEN ({self.remaining_time}s left) -> Continuing normal countdown...")
                    self.preempt_yellow_active = False
                    self._apply_update("GREEN", self.remaining_time)
                    self._run_preempt_tick(self.remaining_time)
            else:
                # Active preemption: let the custom countdown ticker (_run_preempt_tick)
                # manage counting down. We do NOT overwrite the time during countdown.
                pass
        else:
            if self.in_preemption:
                if self.preempt_off_delay_id is None:
                    print("⏳ Preemption signal lost. Debouncing exit (will exit in 4 seconds if no re-detection)...")
                    self.preempt_off_delay_id = self.root.after(4000, self._actual_preempt_exit)

    def _actual_preempt_exit(self):
        """
        Performs the actual preemption state machine restoration after debouncing is completed.
        """
        self.preempt_off_delay_id = None
        if self.in_preemption:
            self.in_preemption = False
            ...
            # Restore the state machine based on original state
            if self.preempt_saved_state == "RED":
                print(f"🔓 Ambulance Passed! Restoring original RED state: {self.preempt_saved_remaining_time}s left")
                self.state = "RED"
                self.remaining_time = self.preempt_saved_remaining_time
                self.sim_index = self.preempt_saved_sim_index
                self.simulation_active = self.preempt_saved_active
                if self.simulation_active:
                    self._sim_tick(self.remaining_time)
                else:
                    self._apply_update(self.state, self.remaining_time)
            elif self.preempt_saved_state in ("YELLOW", "AMBER"):
                if self.state == "GREEN":
                    # Yellow completed during preemption, now we are in the next phase (GREEN).
                    # Resume normal simulation from the current GREEN count.
                    print(f"🔓 Ambulance Passed! Resuming in GREEN phase after yellow completed: {self.remaining_time}s left")
                    self.state = "GREEN"
                    self.sim_index = 2  # Index of GREEN
                    self.simulation_active = self.preempt_saved_active
                    if self.simulation_active:
                        self._sim_tick(self.remaining_time)
                    else:
                    ...
                    # Yellow didn't complete, resume yellow
                    print(f"🔓 Ambulance Passed! Resuming original YELLOW countdown at current count: {self.remaining_time}s left")
                    self.sim_index = self.preempt_saved_sim_index
                    self.simulation_active = self.preempt_saved_active
                    if self.simulation_active:
                        self._sim_tick(self.remaining_time)
                        ...
                        self._apply_update(self.state, self.remaining_time)
            elif self.preempt_saved_state == "GREEN":
                # If originally green, resume normal simulation countdown using the CURRENT remaining time.
                print(f"🔓 Ambulance Passed! Resuming original GREEN countdown at current count: {self.remaining_time}s left")
                self.sim_index = self.preempt_saved_sim_index
                self.simulation_active = self.preempt_saved_active
                if self.simulation_active:
                    self._sim_tick(self.remaining_time)
    ...
    def _run_preempt_tick(self, remaining):
        """
        Custom countdown ticker used exclusively during preemption mode.
        """
        if not self.in_preemption:
            return
        ...
        if self.preempt_yellow_active:
            if remaining <= 0:
                print("🚦 Preemption Yellow finished -> Transitioning to GREEN!")
                self.preempt_yellow_active = False
                self._apply_update("GREEN", 15)
                self.sim_after_id = self.root.after(1000, lambda: self._run_preempt_tick(14))
            else:
                self.sim_after_id = self.root.after(1000, lambda: self._run_preempt_tick(remaining - 1))
        else:
            # We are in GREEN
            if remaining <= 3:
                if self.preempt_off_delay_id is None:
                    # Green reached 3 and ambulance is still present -> loop to 10s
                    print("🚦 Green countdown reached 3s and ambulance is still present -> extending to 10s!")
                    self.preempt_extended = True
                    self._apply_update("GREEN", 10)
                    self.sim_after_id = self.root.after(1000, lambda: self._run_preempt_tick(9))
                else:
                    # During exit debounce (no active ambulance detected), do not extend!
                    print("⏳ Debounce active, lettering green count down past 3 without extending...")
                    if remaining <= 0:
                        self._actual_preempt_exit()
                    else:
                        self.sim_after_id = self.root.after(1000, lambda: self._run_preempt_tick(remaining - 1))
            else:
                self.sim_after_id = self.root.after(1000, lambda: self._run_preempt_tick(remaining - 1))
