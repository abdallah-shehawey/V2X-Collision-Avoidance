...
/*=================================================================================================================*/
/**
 * @fn     GPIO_vSetAlternateFunction
 * @brief : Set the alternate function of a GPIO pin
 * @param : port: GPIO port (PORTA to PORTH)
 * @param : pin: GPIO pin (PIN0 to PIN15)
 * @param : alternateFunction: Alternate function number (AF0 to AF15)
 */
void GPIO_vSetAlternateFunction(GPIO_Port_t port,GPIO_Pin_t pin,GPIO_AlternateFunction_t alternateFunction)
{
  if (port <= GPIO_PORTH)
  {
    if (pin <= GPIO_PIN15)
    {
      GPIO_Port[port]->AFR[pin / 8] &= ~(0xF << ((pin % 8) * 4));
      GPIO_Port[port]->AFR[pin / 8] |= (alternateFunction << ((pin % 8) * 4));
    }
    else
    {
